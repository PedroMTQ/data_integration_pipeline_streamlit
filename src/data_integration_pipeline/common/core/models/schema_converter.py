# source: https://pypi.org/project/pydantic-to-pyarrow/
import datetime
import types
import uuid
...
FIELD_MAP = {
    # TODO change to string_view when this is fixed: pyarrow.lib.ArrowNotImplementedError: Function 'array_take' has no kernel matching input types (string_view, uint16)
    # this was not working when filtering the data with a mask in _write_to_hub_or_link
    str: pa.string(),
    bytes: pa.binary(),
    bool: pa.bool_(),
    float: pa.float64(),
    ...
    NaiveDatetime: pa.timestamp('us', tz='UTC'),
...
LOSING_TZ_TYPES = {
    ...
    AwareDatetime: pa.timestamp('us', tz='UTC'),
}
